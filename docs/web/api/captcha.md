---
title: Captcha 验证码
description: 通过滑块拼图交互完成真人校验，适用于登录、注册、提交等需要降低自动化风险的场景。
isComponent: true
...
缺口数量、目标坐标、拼图块素材、误差阈值、拖动轨迹和校验策略应由技术栈组件或服务端 challenge 数据定义，common 样式层不固定这些规则。单缺口可通过 `--td-captcha-target-left/top/width/height` 与 `--td-captcha-piece-left/top/width/height` 定位；多个缺口可重复渲染 `.t-captcha__cutout` 和 `.t-captcha__piece`，并为每个节点设置独立的 CSS 变量与图片素材。误差阈值建议只用于前端交互反馈，最终通过结果仍应以服务端校验为准。
