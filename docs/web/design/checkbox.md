...
##### 当选项较多、且需要突出选中态时（如 CRM/OA 系统的权限分配、功能开关配置），可使用按钮风格的多选框组。选项以标签式按钮排列，选中项以深色填充突出，未选中项为浅色描边，点击区域更大，便于快速定位已选项。

##### 按钮风格提供三种形态：描边（outline，选中项为主色描边 + 浅色底）、填充（filled，浅色容器底 + 凸起选中块）、主色填充（primary-filled，选中项为主色底反色字）。选项较多时可纵向排列。
