...
Use `--td-tab-bar-glass-bg-color`, `--td-tab-bar-glass-border-color`, and `--td-tab-bar-glass-shadow` to customize the stable material styles. `--td-tab-bar-glass-fallback-blur` controls the Gaussian blur radius when SVG enhancement is unavailable. The selected capsule on a round TabBar is controlled by `--td-tab-bar-selected-bg-color` and `--td-tab-bar-selected-bg-opacity`; the latter uses a percentage. Refraction strength and texture parameters remain internal in the initial API.
