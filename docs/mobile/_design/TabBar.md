...
液态玻璃适用于需要保留背景上下文的底部导航。复杂背景或明暗主题下，标签文字、图标和选中态仍应清晰可辨，材质表现不得削弱导航信息的可读性。

`normal` 形态保持原有矩形轮廓。原有 `round + tag` 悬浮标签栏启用液态玻璃后，只同步背景折射、边缘高光等材质效果，不改变既有布局与排版；逐项选中态改用与共享胶囊一致的半透明选中背景，以保留底层内容上下文。

新增的共享胶囊样式通过独立主题启用，使用屏幕内缩的悬浮布局、紧凑的图标文字排布和可移动的同心选中胶囊。液态玻璃不改变导航语义或点击区域。
